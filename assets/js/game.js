...
var fightOrSkip = function() {
    ...
    // Conditional Recursive Function Call (makes this function repeat or loop)
    // if (!promptFight) { } = invalid or null value
    if (promptFight === "" | promptFight === null) {
        window.alert("You need to provide a valid answer! Dumb bitch! Try again!");
        return fightOrSkip();
    }
...
var fight = function(enemy) {
    ...
    while(playerInfo.health > 0 && enemy.health > 0) {
        if (isPlayerTurn) {
            // ask plater if they'd like to fight or skip using fightOrSkip function 
            if (fightOrSkip()) {
...
var getPlayerName = function() {
    ...
    while (name === "" | name === null) {
        name = prompt("Bitch quit playing games! What is your robot's name?");
    }
...
var endGame = function() {
    window.alert("The game has now ended. Let's see how you did, bitch.");

    // check localStorage for high score, if it's not there, use 0
    var highScore = localStorage.getItem("highscore");

    // truthy falsy statement assigns 0 if highScore is false/null
    ...
    if (playerInfo.money > highScore) {
        ...
        alert(playerInfo.name + " did not beat the high score of " + highScore + ". Better luck next time loser!");
    }
